...
# 1. Load dataset_negative_deviation.csv
csv_path = "dataset_negative_deviation.csv"
...
actual_vs_pred_path = os.path.join(
    output_dir, "actual_vs_predicted_negative_deviation.png"
)
...
residuals_plot_path = os.path.join(
    output_dir, "residuals_plot_negative_deviation.png"
)
...
try:
    ...
    subprocess.run(
        [
            ...
            "commit",
            "-m",
            f"Add negative deviation Actual vs Predicted & Residuals plots (MAE: {mae:.2f}, MSE: {mse:.2f}, R2: {r2:.3f})",
        ],
        check=True,
    )
