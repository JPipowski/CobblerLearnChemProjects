...
# 1. Load dataset_high_scatter.csv
csv_path = "dataset_high_scatter.csv"
...
actual_vs_pred_path = os.path.join(
    output_dir, "actual_vs_predicted_high_scatter.png"
)
...
residuals_plot_path = os.path.join(output_dir, "residuals_plot_high_scatter.png")
...
try:
    ...
    subprocess.run(
        [
            ...
            "commit",
            "-m",
            f"Add high scatter Actual vs Predicted & Residuals plots (MAE: {mae:.2f}, MSE: {mse:.2f}, R2: {r2:.3f})",
        ],
        check=True,
    )
