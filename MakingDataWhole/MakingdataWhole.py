...
# Scale features so Euclidean distance isn't dominated by high-variance features like 'fare'
scaler = StandardScaler()
df_scaled = pd.DataFrame(
    scaler.fit_transform(df_numeric), columns=features, index=df_numeric.index
)

# 3. Perform KNN Imputation on full scaled dataset
imputer = KNNImputer(n_neighbors=5)
df_knn_imputed_scaled = imputer.fit_transform(df_scaled)
...
print("--- Correlation Matrix (After KNN Imputation) ---")
...
plt.title("Correlation Matrix (Post-KNN Imputation)", fontsize=12)
plt.tight_layout()
...
print(f"Mean Absolute Error (MAE) for KNN Age Imputation: {mae:.2f} years")
...
plt.title(
    f"KNN Imputation Evaluation: Actual vs. Predicted Age (MAE: {mae:.2f})",
    fontsize=12,
)
plt.xlabel("Actual Age", fontsize=10)
plt.ylabel("KNN Predicted Age", fontsize=10)
plt.legend()
plt.grid(True, linestyle=":", alpha=0.6)
plt.tight_layout()

eval_plot_path = os.path.join(
    output_dir, "knn_age_prediction_actual_vs_predicted.png"
)
...
try:
    ...
    subprocess.run(
        [
            ...
            "commit",
            "-m",
            f"Add correlation heatmap and KNN age evaluation plot (MAE: {mae:.2f})",
        ],
        check=True,
    )
